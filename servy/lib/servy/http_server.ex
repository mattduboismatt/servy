...
# Transcoded to elixir - initial... doesn't respond to the client or loop back to the one listener
# defmodule Servy.HttpServer do
#   def server do
#     # use a keyword list because we have list of tuples with atoms as first item
#     {:ok, lsock} = :gen_tcp.listen(5678, [:binary, packet: 0, active: false])

#     # blocking call that waits to accept a client connection on the listening socket
#     # when established, bound to the new client socket
#     {:ok, sock} = :gen_tcp.accept(lsock)

#     # receive the request from the client socket
#     # bin is bound to request string
#     # change to zero to receive all available bytes
#     {:ok, bin} = :gen_tcp.recv(sock, 0)

#     # send response back to client and loop back to request line to accept another request
...
defmodule Servy.HttpServer do
  ...
  def accept_loop(listen_socket) do
    ...
    # Need to keep track of the controlling process so that we can close
    # all spawned processes when the controller process dies.
    # This makes the client_socket the controlling process for our spawned process.
    # Now if the spawned process dies, the client_socket will be automatically closed.
